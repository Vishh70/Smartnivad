...
export function PriceDropAlert({ dealId, currentPrice }: PriceDropAlertProps) {
  ...
  if (status === "success") {
    return (
      <GlassCard className="p-4 flex flex-col items-center justify-center text-center bg-green-50/50 border-green-200">
        ...
        <p className="text-sm text-gray-600 font-medium">We'll email you when the price drops to ₹{targetPrice}.</p>
      </GlassCard>
    );
  }
  ...
  return (
    <GlassCard className="p-5 relative overflow-hidden">
      ...
      {status === "error" && (
        <p className="text-xs text-red-500 mt-2 font-bold text-center">Failed to set alert. Please try again.</p>
      )}
    </GlassCard>
  );
}
