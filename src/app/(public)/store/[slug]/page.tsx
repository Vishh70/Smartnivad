...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `${store.name} Deals & Coupons — TechDeals AI`,
...
export default async function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <GlassCard className="mb-12 !p-8 sm:!p-12 flex flex-col sm:flex-row items-center gap-8">
        ...
        {store.website && (
          <a href={store.website} target="_blank" rel="noopener noreferrer">
            <GlowButton variant="secondary" className="flex items-center gap-2 shrink-0">
      ...
      {coupons.length > 0 && (
        <div className="mb-16">
          <SectionHeading title={`${store.name} Coupons`} subtitle="Verified promo codes — click to copy." />
              ...
              <GlassCard key={coupon.id} className="!p-0 overflow-hidden">
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-sm font-bold text-gray-900 leading-snug flex-1 mr-3 line-clamp-2">{coupon.title}</h3>
                    <span className="text-xs font-bold text-green-400 bg-green-400/10 px-2 py-1 rounded-lg border border-green-400/20 shrink-0">{coupon.discountText}</span>
                  </div>
                  <div className="flex gap-2 mt-4">
                    ...
                    <Link href="/coupons" className="flex items-center justify-center w-11 rounded-xl bg-[var(--color-primary)] text-white text-xs hover:bg-purple-500 transition-colors font-semibold">
                      Copy
                    </Link>
                  </div>
                </div>
      ...
      {/* Active Deals */}
      <div>
        <SectionHeading title={`Latest ${store.name} Deals`} subtitle="All active offers from this store." />
        {deals.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 mt-6 sm:mt-10">
            {deals.map(deal => <ProductCard key={deal.id} product={deal} />)}
          </div>
        ) : (
          <div className="text-center py-16 text-gray-500">No deals from this store right now.</div>
        )}
      </div>
