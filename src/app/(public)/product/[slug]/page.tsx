...
const PriceHistoryChart = dynamic(() => import("@/components/ui/PriceHistoryChart").then(mod => mod.PriceHistoryChart));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: product.seoTitle || `${product.title} - Best Deals & Reviews | TechDeals AI`,
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedProducts = relatedProductsRaw.filter(p => p.id !== product.id).slice(0, 8);
  ...
  const origLabel     = product.originalPrice ? `₹${formatNumber(product.originalPrice)}` : "";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 h-fit lg:sticky lg:top-24 space-y-6">
            <div className="relative aspect-square rounded-3xl overflow-hidden bg-white/60 border border-gray-100 shadow-[0_8px_32px_rgba(0,0,0,0.06)] group p-8 flex items-center justify-center backdrop-blur-xl">
              ...
              <button className="absolute top-4 right-4 bg-white/80 hover:bg-white text-gray-500 hover:text-blue-600 p-3 rounded-full backdrop-blur-md transition-all z-10 shadow-sm">
                <Share2 size={20} />
              </button>
              {product.imageUrl?.startsWith("http") ? (
                <div className="relative w-full h-full transform transition-transform duration-500 group-hover:scale-105">
                  <Image src={product.imageUrl} alt={product.title} fill sizes="(min-width: 1024px) 42vw, 100vw" className="object-contain drop-shadow-2xl" priority />
                </div>
              ) : (
                <div className="flex items-center justify-center text-gray-400 w-full h-full font-medium">No Image Available</div>
              )}
            </div>
          ...
          <div className="lg:col-span-7 flex flex-col pt-4">
            <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4 leading-tight tracking-tight">{product.title}</h1>
            ...
            <div className="flex items-center space-x-3 mb-6">
              ...
              <span className="text-sm text-gray-500 font-medium">AI-Verified Review</span>
            </div>
            ...
            <GlassCard className="!p-6 mb-6 border-l-4 border-l-blue-600">
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                  <p className="text-sm text-gray-500 mb-1 uppercase tracking-wider font-bold">Deal Price</p>
                  <div className="flex items-baseline gap-3">
                    <span className="text-4xl font-black text-gray-900">{priceLabel}</span>
                    {origLabel && <span className="text-xl text-gray-400 line-through">{origLabel}</span>}
                    <span className="text-sm font-bold text-green-600 bg-green-50 px-2.5 py-1 rounded-lg border border-green-200">{product.discount}% OFF</span>
                  </div>
                </div>
              ...
              {hasExpiry && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <CountdownTimer expiresAt={product.expiresAt!} variant="prominent" />
                </div>
              )}

              <div className="mt-6">
                {product.affiliateUrl !== "#" ? (
                  <a href={product.affiliateUrl} target="_blank" rel="noopener noreferrer" className="block w-full rounded-2xl gradient-btn px-6 py-5 text-center text-xl font-bold tracking-wide shadow-xl hover:-translate-y-1 transition-all">
                    GET DEAL NOW
                  </a>
                ) : (
                  <p className="rounded-xl bg-gray-50 px-4 py-3 text-center text-sm text-gray-500 font-medium">This deal link is not available yet.</p>
                )}
            ...
            <div className="mb-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <PriceHistoryChart dealId={product.id} currentPrice={product.currentPrice} />
              </div>
              <div>
                <PriceDropAlert dealId={product.id} currentPrice={product.currentPrice} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
              <GlassCard className="!p-6 !bg-green-50/50 !border-green-200">
                <h3 className="text-xl font-bold text-green-700 mb-4 flex items-center"><CheckCircle className="mr-2" size={20}/> Why Buy It</h3>
                <ul className="space-y-3">
                  {prosList.map((pro, idx) => (
                    <li key={idx} className="text-gray-700 flex items-start font-medium"><span className="text-green-500 mr-3 mt-0.5 font-bold">✓</span>{pro}</li>
                  ))}
                </ul>
              </GlassCard>
              <GlassCard className="!p-6 !bg-red-50/50 !border-red-200">
                <h3 className="text-xl font-bold text-red-700 mb-4 flex items-center"><XCircle className="mr-2" size={20}/> Reasons to Avoid</h3>
                <ul className="space-y-3">
                  {consList.map((con, idx) => (
                    <li key={idx} className="text-gray-700 flex items-start font-medium"><span className="text-red-500 mr-3 mt-0.5 font-bold">×</span>{con}</li>
                  ))}
                </ul>
              </GlassCard>
            </div>
                ...
                <table className="w-full text-left text-gray-700">
                  <tbody className="divide-y divide-gray-100">
                    <tr className="hover:bg-gray-50/50 transition-colors">
                      <th className="px-6 py-4 font-bold w-1/3 text-gray-500 text-sm">Brand</th>
                      <td className="px-6 py-4 font-semibold">{product.brand?.name ?? "—"}</td>
                    </tr>
                    <tr className="hover:bg-gray-50/50 transition-colors">
                      <th className="px-6 py-4 font-bold w-1/3 text-gray-500 text-sm">Category</th>
                      <td className="px-6 py-4 font-semibold capitalize">{categoryName}</td>
                    </tr>
                    <tr className="hover:bg-gray-50/50 transition-colors">
                      <th className="px-6 py-4 font-bold w-1/3 text-gray-500 text-sm">Rating</th>
                      <td className="px-6 py-4 font-semibold">{product.rating} / 5.0</td>
                    </tr>
                    <tr className="hover:bg-gray-50/50 transition-colors">
                      <th className="px-6 py-4 font-bold w-1/3 text-gray-500 text-sm">Total Views</th>
                      <td className="px-6 py-4 font-semibold">{formatNumber(product.views)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16 pt-16 border-t border-gray-100">
            <SectionHeading title="Related Deals" subtitle="Other popular offers right now." />
      ...
      <div className="fixed bottom-16 lg:bottom-0 left-0 w-full z-[60] bg-white/90 backdrop-blur-2xl border-t border-gray-200 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="hidden sm:flex items-center gap-4">
            ...
            <div>
              <p className="text-gray-900 font-bold line-clamp-1">{product.title}</p>
              <p className="text-blue-600 font-black text-lg">{priceLabel}</p>
            </div>
          </div>
          <div className="flex-1 sm:flex-none flex justify-end">
            {product.affiliateUrl !== "#" && (
              <a href={product.affiliateUrl} target="_blank" rel="noopener noreferrer" className="w-full rounded-xl gradient-btn px-8 py-3 text-center text-lg font-bold shadow-lg sm:w-64">
                GET DEAL
              </a>
            )}
          </div>
