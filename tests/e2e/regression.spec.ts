...
test.describe("SmartNivad Regression Suite", () => {
  ...
  test("Wishlist functionality", async ({ page }) => {
    ...
    await saveBtn.click();
    // Wait for the state to persist to localStorage/cookie
  ...
  test("Product page rendering", async ({ page }) => {
    ...
    if (productUrl) {
      ...
      await expect(
        page.getByRole("link", { name: /get deal|view deal|buy/i }).first(),
      ).toBeVisible();
    }
  });
  ...
  test("Authentication routing", async ({ page }) => {
    ...
    await expect(
      page.getByText(/SmartNivad Admin|Sign in to manage/i).first(),
    ).toBeVisible();
